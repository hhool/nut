...
static void *zspace_battery_scale(USBDevice_t *device)
{
...
static usb_device_id_t zspace_usb_device_table[] = {
	/* Zspace */
	{ USB_DEVICE(ZSPACE_VENDORID, 0x0101), &zspace_battery_scale },
 ...
 */
static void zspace_adjust_battery_scale(double batt_volt)
{
...
static const char *zspace_battvolt_fun(double value)
{
	static char	buf[8];

	if(might_need_battery_scale) {
		zspace_adjust_battery_scale(value);
	}
...
static info_lkp_t zspace_battvolt[] = {
	{ 0, NULL, &zspace_battvolt_fun }
};

/* returns statically allocated string - must not use it again before
   done with result! */
static const char *zspace_battcharge_fun(double value)
{
	static char	buf[8];
...
static info_lkp_t zspace_battcharge[] = {
	{ 0, NULL, &zspace_battcharge_fun }
};
...
static hid_info_t zspace_hid2nut[] = {
  ...
  { "battery.charge", 0, 0, "UPS.PowerSummary.RemainingCapacity", NULL, "%s", 0, zspace_battcharge },
  ...
  { "battery.voltage", 0, 0, "UPS.PowerSummary.Voltage", NULL, "%s", 0, zspace_battvolt },
